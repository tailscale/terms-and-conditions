...
| Software development | The core functionality of the Tailscale Solution, including the coordination server, logging infrastructure, DERP relay servers, and clients, are written in [Go](https://go.dev/). Go is a language that provides automatic memory management, and so doesn’t rely on the developer to allocate and free up memory — which prevents a [whole class of memory safety vulnerabilities](https://cwe.mitre.org/data/definitions/119.html).<br><br>All source code changes require a peer review and we regularly conduct audits of our source code. We have identity access management with SSO and MFA enabled for our production systems, and access permissions are reviewed quarterly. |
